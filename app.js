(() => {
  ...
  function updateEntitlement() {
    const pro = FS.hasPro();
    els.entitlementPill.textContent = pro ? "PRO ACTIVE" : "FREE DEMO";
    els.settingsPlanBadge.textContent = pro ? "PRO ACTIVE" : "FREE DEMO";
  ...
  function showWalkthrough(step = 0) {
    state.walkthroughStep = clamp(step, 0, 2);
    renderWalkthrough();
  ...
  function renderWalkthrough() {
    ...
    els.walkthroughNext.textContent =
      state.walkthroughStep === 2 ? "I understand — continue" : "Next";
  }
  ...
  async function activateCode() {
    ...
    try {
      const ok = await FS.verifyAccessCode(els.accessCodeInput.value);
      if (!ok) {
        els.accessMessage.textContent = "That code is not valid.";
        return;
      }
      els.accessMessage.textContent = "Pro access is active on this browser.";
      els.accessCodeInput.value = "";
      updateEntitlement();
      closeModal(els.paywallModal);
      toast("FaceScore Pro unlocked.");
    } catch {
  ...
  function openPaywall() {
    document
      ...
      .forEach((el) => (el.textContent = CFGX.PRICE || "$19"));
    document
  ...
  async function processFrame(now) {
    ...
    try {
      await state.model.send({ image: els.video });
      state.recoveryAttempts = 0;
    } catch (error) {
  ...
  document.addEventListener("facescore-consent", () => persistPreferences());
